# Wave 1 specification — Test Reliability

## 1. Objective

Оценить, защищают ли существующие тесты фактическое поведение; отличить зелёные проверки от надёжной regression protection без изменения тестов.
...
Основные IDs по mapping: **93, 95, 117**. Связанные: 94 — только планирование мутаций; 21, 35, 37, 62, 70, 73, 96 — только test/CI traceability. Section 0 читай при необходимости для контекста принципов, не как разрешение новых действий.
...
- Test inventory, все frontend tests, auth/session/refresh/logout, ownership/cross-user и transaction/rollback.
- Минимум одна representative suite каждого backend domain; suites с большим mocking.
- Weak/tautological assertions, happy-path-only, implementation-detail tests, expected logic duplication.
- Flaky/order-dependent tests, shared fixtures, DB isolation, nondeterministic time/randomness, skipped cases и CI/local differences.

## 10. Required coverage

- Mandatory: инвентаризировать suites и backend domains по исходникам; прочитать все frontend tests; глубоко проверить auth/session/refresh/logout, ownership/cross-user и transaction/rollback; минимум одну representative suite каждого backend domain.
- Для domain без suite отметить отсутствие тестовой защиты и границы поиска; не создавать новую suite. Если обязательная выборка не завершена, явно отметить incomplete coverage, не заявлять выполнение.
- Deep-dive hotspots: heavy mocking, always-true assertions, fixtures/shared state и ожидания, повторяющие production algorithm; сверять тест с защищаемым кодом.
- Explicit blind spots: реальные mutation/fault experiments, e2e/DB runtime без безопасного окружения, нагрузка и исчерпывающая проверка всех backend suites сверх обязательной выборки.
...
- Построй test-to-domain/invariant matrix с именами всех рассмотренных suites; укажи критерии representative selection и почему assertion поймает неправильное поведение.
- Сопоставь существующие CI/local scripts с baseline на текущем SHA; исторические counts и PASS не выдавай за текущую проверку. Unit/mock результат не доказывает DB transaction/rollback.
- Верни mutation candidates: предполагаемая мутация, тест, который должен упасть, причина, безопасность/польза и требуемое isolated окружение. Ничего не мутируй и не выполняй fault injection.
...
Test inventory/coverage matrix; список всех frontend tests и representative backend suites; mutation candidates без экспериментов; CI/local differences.

## 14. Candidate finding rules

Используй prefix `TEST` и detailed template `03-FINDINGS.md`; согласуй свободный диапазон ID с координатором до назначения, не меняй существующие IDs. Возвращай только Candidate Findings вне registry; observed evidence не равно независимо Confirmed finding.
...
```text
В новом независимом conversation выполни только Test Reliability по specification docs/audits/yuni-2026-09/prompts/wave-1/02-TEST-RELIABILITY.md из выделенного worktree.
...
Пиши только docs/audits/yuni-2026-09/passes/wave-1/02-TEST-RELIABILITY.md. Не редактируй registry. Сохрани отчёт и worktree для Coordinator; не выполняй cleanup.
```
