# Wave 1 specification — Security and Data Integrity

## 1. Objective

Проверить критические auth/owner/data boundaries по коду и существующим тестам, сформировать policy matrix и кандидатов рисков без активных атак.
...
Основные IDs по mapping: **25, 26, 29, 35, 36, 37, 38, 58, 60, 62, 63, 64, 65, 70, 73, 98**. Связанные: 12, 19, 20, 59, 61, 66, 68, 69, 99, 100 — только критические auth/data срезы; 51, 90, 91, 101 — только presence gate условных компонентов. Section 0 читай при необходимости для контекста принципов, не как разрешение новых действий.
...
- Entry-point и credential/token inventory; registration/login, password handling, access/refresh separation, JWT validation, refresh rotation и session storage.
- Logout/revocation, authorization/ownership и User A → User B resources.
- DTO/input validation, serializers/output, rate limits, uploads, secrets/configuration без вывода значений.
- Critical Prisma constraints, transaction/data-integrity boundaries, race-condition и partial-write candidates.
- WSS, Redis, queues, product AI, payments, webhooks, replicas, admin и аналоги сначала проверить на наличие, не расширять автоматически полный security scope.

## 10. Required coverage

- Mandatory: inventory entry points и видов credentials без значений; ограниченная auth/session policy matrix, проверки access/refresh различения, refresh/logout, ownership, input/files/output/limits и критических Prisma/transaction invariants по source/existing tests.
- Deep-dive hotspots: refresh rotation и revoked credentials, cross-user resource paths, media serializers/uploads, partial writes и конкурентные state transitions как кандидаты, без destructive concurrency execution.
- Explicit blind spots: exploit/runtime security validation, production/staging, новая attack/e2e suite, полноценный WSS/AI/admin аудит, полная DB/consistency matrix и будущая production topology.
...
- Построй матрицу: сценарий → ожидаемое поведение access/refresh → источник policy → code/test evidence → результат/uncertainty. Не выводи требуемую policy только из текущей реализации.
- Неопределённые logout/revocation/role-change правила пометь `Owner Decision Required`; приостанови зависящий вывод и продолжи независимый безопасный анализ. Не навязывай instant revoke или конкретную token-family модель.
- Проследи guards/DTO/serializers/schema и критические write boundaries; перечисли race/partial-write candidates для следующей валидации. Runtime security validation, active attacks и новые exploit tests здесь запрещены.
...
Entry-point/credential inventory; auth/session policy matrix и Owner Decision Required; critical data invariants; runtime/race/partial-write проверки только как предложения.

## 14. Candidate finding rules

Используй prefix `SEC` и detailed template `03-FINDINGS.md`; согласуй свободный диапазон ID с координатором до назначения, не меняй существующие IDs. Возвращай только Candidate Findings вне registry; observed evidence не равно независимо Confirmed finding.
...
```text
В новом независимом conversation выполни только Security and Data Integrity по specification docs/audits/yuni-2026-09/prompts/wave-1/03-SECURITY-DATA-INTEGRITY.md из выделенного worktree.
...
Пиши только docs/audits/yuni-2026-09/passes/wave-1/03-SECURITY-DATA-INTEGRITY.md. Не редактируй registry. Сохрани отчёт и worktree для Coordinator; не выполняй cleanup.
```
