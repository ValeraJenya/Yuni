...
| Database-name guard | Parse both DB URLs before Compose and before Prisma; require `yuni_validation_test`. Also inspect the effective Compose config without printing secret values and verify `POSTGRES_DB` matches. |
...
No Docker service, PostgreSQL connection, migration, seed, reset/drop, media write, runtime test, or application listener was started. No real environment file, credentials, secrets, or user data were read. Production code and project configuration were not changed.
