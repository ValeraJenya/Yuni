...
- Backend имеет NestJS foundation, health endpoint, auth/session flow и Profiles MVP. Есть конфигурация, ValidationPipe, CORS, Helmet, throttling foundation, PrismaModule, CurrentUser decorator и JWT access guard.
...
- Product-модуль `profiles` имеет MVP controller/service/DTO для own profile read/update и public profile by handle. `media`, `likes`, `matches`, `chat`, `moderation` пока только scaffold. Это полезные границы модульного монолита, но для них ещё нет controllers, services, DTO, owner checks, membership checks, serializers и tests.
