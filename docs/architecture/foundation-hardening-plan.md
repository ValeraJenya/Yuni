...
- Frontend сейчас является prototype/mock/demo flow. Основные экраны и UI уже есть, но auth, discover, matches, messages и profile используют mock-data/demo state вместо реального backend API.
- Backend имеет NestJS foundation, health endpoint и минимальный auth/session flow. Есть конфигурация, ValidationPipe, CORS, Helmet, throttling foundation, PrismaModule, CurrentUser decorator и JWT access guard.
...
- Product-модули `profiles`, `media`, `likes`, `matches`, `chat`, `moderation` пока только scaffold. Это полезные границы модульного монолита, но в них ещё нет controllers, services, DTO, owner checks, membership checks, serializers и tests.
- Docker/local infra и CI пока не закреплены полностью. `infra` существует как зона ответственности, но локальный PostgreSQL workflow и обязательные quality gates ещё нужно оформить.
...
1. Frontend lint/typecheck/build quality gates.
2. Убрать `ignoreBuildErrors` в Next config.
3. Backend auth tests.
4. CI workflow.
5. `docker-compose` для локального PostgreSQL.
6. Monitoring/logging/error tracking позже.
7. Email verification/reset password позже.
8. Admin/moderation tools позже.
...
Что входит:
- починить frontend lint;
- убрать `ignoreBuildErrors` после стабилизации typecheck;
- добавить backend/frontend обязательные checks;
- добавить минимальный CI workflow.

Acceptance criteria:
- перед merge выполняются install, prisma validate, backend lint/build, frontend lint/typecheck/build и tests;
