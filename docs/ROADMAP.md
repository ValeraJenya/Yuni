...
| 080 | Инпут поиска в `/messages` без `id`/`name` | review | P3 | — | Ветка `fix/task-079-public-console` запушена, PR не открыт, отдельный коммит от 079. `apps/frontend/app/(app)/messages/page.tsx:329` — добавлены `id="messages-search"` и `name="messages-search"`. Evidence — `docs-site/content/tasks/080.md`. |
