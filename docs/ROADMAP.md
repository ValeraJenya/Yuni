...
ручной проверке сервиса 14 августа: 069 (редактирование профиля) смерджен,
070 (онбординг) закрыт патчем и ждёт merge. Задача 067 разделена: 067a
(экспорт данных) готов и ждёт merge, следом 067b (удаление аккаунта).

| ID | Название | Статус | Комментарий |
|---|---|---|---|
| 070 | Онбординг-анкета после регистрации | review | Патч готов, ждёт merge |
...
| 070 | Онбординг-анкета после регистрации | review | P1 | Task 069 | После `/join` пользователя отправляет в `/discover`, где пусто: профиль не заполнен, в выдаче он не участвует. Нужна анкета сразу после регистрации со всеми полями, обязательные помечены звёздочкой. Переиспользует форму из Task 069 |
