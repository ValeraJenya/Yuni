...
одной веткой `fix/task-074-hero-cta` — PR не открыт, открывает владелец.
...
`CLAUDE.md:9` теперь расходится со старой формулировкой — правка отдельным
заходом, требует подтверждения владельца. PR не открыт.
