...
| 079 | 401 в консоли на каждой публичной странице | review | P2 | — | Ветка `fix/task-079-public-console` запушена, PR не открыт. `AuthProvider` больше не запускает `refreshSession()` сам при монтировании — bootstrap переехал в `useAuth()`: любой потребитель контекста инициирует его в собственном `useEffect`, идемпотентно (ref-guard в `AuthProvider`). `/`, `/terms`, `/privacy`, `/help` не читают сессию и теперь не делают запрос вообще; `/join`, `/signin`, `/forgot-password` по-прежнему делают ровно один — им нужен `isLoading`/`isAuthenticated` для редиректа уже залогиненных. Полная evidence — `docs-site/content/tasks/079.md`. |
