...
| 074 | Главный CTA лендинга ниже сгиба | done | P1 | Ветка `fix/task-074-hero-cta`; PR не открыт — открывает владелец. Замер `getBoundingClientRect()` по слагаемым (`pt-28` 112px, eyebrow+`mb-14` 56px, заголовок 454px, `mt-16` 64px, tagline+sub+`gap-8`×2, CTA 48px, `pb-20` 80px = 986px стек) показал переизбыток именно в нижних отступах, не в заголовке. Убраны `md:`-надбавки (`mb-14`→`mb-6`, `mt-16`→`mt-8`, `gap-8`→`gap-4`, `md:pb-20` снят) и снижен `pt-28`→`pt-20`, `justify-start` не тронут. `cta.bottom` на 1280×800 — было 907 (за сгибом на 107px), стало 778 (запас 22px); на 1280×1100 `eyebrow.top` не изменился (80px и там, и на 800/900) — пустой промежуток под навбаром (дефект `3a196a3`) не вернулся; 390×844 не пострадал (`cta.bottom` 560→504, всё ещё далеко от края). Проверено на всех четырёх вьюпортах в chrome-devtools-mcp на реальном dev-сервере, скриншоты до/после — `docs-site/content/tasks/074.md`. |
