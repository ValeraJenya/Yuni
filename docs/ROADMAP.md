...
Открытых P0 нет. Группа P1 по конкурентности и приватности закрыта целиком
(042, 044, 046, 047). Текущий приоритет — продуктовые пробелы, найденные при
ручной проверке сервиса 14 августа: 069 (редактирование профиля) и 070
(онбординг), затем 067 (удаление аккаунта и экспорт данных).
...
| 046 | Неатомарные write-пути | in_progress | Регистрация закрыта; like → match и два пути уведомлений остаются |
...
| 046 | Неатомарные write-пути | in_progress | P1 | — | Часть 1 (регистрация) закрыта в PR #63. Остаются like → match, match → notifications и message → notification. Outbox не нужен: уведомления — строки в той же БД, чинится общей транзакцией с проброшенным `tx`. Техзадание передано отдельно |
...
| 071 | Маскоты вместо чужого фото в фолбэках | idea | P2 | — | `/hero-portrait.jpg` — фотография модели с лендинга — используется заглушкой в шести местах: профиль, Discovery (дважды), сообщения, `discovery-api`, мок-данные. Лицо одного человека становится аватаром любого пользователя без фото. Заменить на векторных кроликов с выбором по полю `gender` |
